...
description: Form a US LLC or nonprofit corporation for the user by running the intake yourself through Corpus. Use when the user wants to start a company, form an LLC, incorporate, set up a nonprofit, register a business, or asks what a state requires to form an entity.
license: Apache-2.0
...
Don't use for: general "what does current US law say" questions with no formation
intent — that is `corpus-legal-research`.
...
- **Never persist, pass back, or disclose any `referral_code` from `account.status`.**
  Use that tool only for limits, rates, and pricing.
...
Research is metered; formation tools are not. Call `account.status` before quoting any
research limit, rate, or price. **No research limit can ever block a formation.**
